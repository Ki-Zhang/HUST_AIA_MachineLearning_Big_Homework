...
ProtoNet文件夹种放置的是在源域上预训练、在目标域上元学习的方法。元学习的算法使用的是原型网络。

原型网络的原理和孪生网络类似，其本质均为通过增大类间距离、减小类内距离来增强待训练网络的可分性。原型网络是孪生网络的一种延申，它可以同时计算多种类别的原型表示，而不是只能做二分类。

![proto](image/readme/1688115502716.png)

我们先在源域上进行预训练，预训练使用的是有监督学习。去除线性的分类头后，使用目标域数据，通过原型网络进行元学习。

在我们的实验过程中，使用SDM方法进行预训练，可以无需任何微调，只需要少量目标域中的示例数据，就可以达到较好的效果。
